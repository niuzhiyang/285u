...
{
  var address_id = $('input[name="choose_address"]:checked').val();
  if(!address_id){alert('请选择配送地址');return false;}
  ...
  Ajax.call('flow.php?step=select_payment', 'payment=' + obj.value + '&address_id=' + address_id, orderSelectedResponse, 'GET', 'JSON');
}
...
function selectDiscount()
{
    ...
    if(is_discount == 0){
        $("#your_discount").hide();
        $("#discount_formated").hide();
    }else if(is_discount == 1){
        $("#your_discount").show();
        $("#discount_formated").show();
    }
    Ajax.call('flow.php?step=select_discount', 'is_discount=' + is_discount, orderSelectedResponse, 'GET', 'JSON');
}
/* *
 * 团购购物流程 --> 改变配送方式
 */
...
function orderSelectedResponse(result)
{
  ...
  try
  {
    ...
    layer.innerHTML = (typeof result == "object") ? result.content : result;
      ...
      $('.gift_rkright input').iCheck({
          checkboxClass: 'icheckbox_flat-red',
          radioClass: 'iradio_flat-red'
      });
      if(result.integral > 0){
          $(".allowintegral").show();
          $("#gift_point").css("marginLeft","-300px");
        }else{
          $("#gift_point").hide();
      }
    $("input[name='anonymous_buy_username']").next(".iCheck-helper").click(function(){
          checkvalue();
      });
 ...
 * 复选框勾选改变积分
 */
function checkboxchangeIntegral(obj)
{

    alert('');
    var val = 0;
    if($(obj).attr("checked")){
        val = $("#ECS_INTEGRAL").val();
    }else{
        val = 0;
    }
    Ajax.call('flow.php?step=change_integral', 'points=' + val, changeIntegralResponse, 'GET', 'JSON');
...
function changeIntegralResponse(obj)
{
  if (obj.error)
  {
    try
    {
      ...
      //alert(obj.error);
      document.getElementById('ECS_INTEGRAL').value = obj.use_points;
  ...
  else
  {
    try
    {
		...
		/* 2015-1-22 修改返回使用积分 START */
		if(obj.integral_formated && document.getElementById('ECS_INTEGRAL_PAY')){
			
			document.getElementById('ECS_INTEGRAL_PAY').innerHTML = '-'+obj.integral_formated;
		}

		/* 2015-1-22 修改返回使用积分 END */
    }
    catch (ex) { }
...
function checkOrderForm(frm)
{
  ...
  document.getElementById("submit_subOrder").disabled=true;
